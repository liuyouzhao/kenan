...
bool Color::parseHexColor(const std::string& name, RGBA32& rgb)
{
...
static int parseInt(const std::string& s, unsigned& i)
{
    int res=0;
    int a = 1;
...
static float parseFloat(const std::string& s, unsigned& i)
{
    int res = 0;
    float a = 1.0;
    if (s[i] == '-')
    ...
    float ans = res, cx = 10.0;
    if (s[i] == '.')
        ++i;

    while (isNumber(s[i]))
    {
        ans += (float)toInt(s[i++])/cx;
        cx *= 10.0f;
    }
    return a * ans;
}

bool parseRGBAColor(const std::string& name,unsigned i, RGBA32& m_color)
{
    unsigned j;
    int rgb[4];
    bool isa = ('a' == name[3]);
    float ta = 10.0, alp = 1.0;
    ...
    for(j = 0; j < 3; ++j)
    {
        ...
        if (-1 == (rgb[j] = parseFloat(name,i)))//ERROR
            return false;
    ...
    if ('a' == name[3])
    {
        ...
        if (0.0 > (alp = parseFloat(name,i)))//ERROR
            return false;
    }
    rgb[3] = rgb[3] * alp;
    ...
    for (j = 0; j < 4; ++j)
    {
        rgb[j] = rgb[j] < 0 ? 0 : rgb[j];
        rgb[j] = rgb[j] > 255 ? 255 : rgb[j];
    }
    
    m_color = max(0, min(rgb[3], 255)) << 24 | max(0, min(rgb[0], 255)) << 16 | max(0, min(rgb[1], 255)) << 8 | max(0, min(rgb[2], 255));
    return true;
}
...
Color::Color(const std::string& name)
{
    if (name[0] == '#')
    {
        m_valid = parseHexColor(name.substr(1), m_color);
    }
    else if ((name[0] == 'r') && (name[1] == 'g') && (name[2] == 'b'))
    {
        if (name[3] == 'a' && name[4] == '(')
            m_valid = parseRGBAColor(name, 5, m_color);
        else if ('(' == name[3])
            m_valid = parseRGBAColor(name, 4, m_color);
        else m_valid = false;
    }
    else
    {
...
std::string Color::name() const
{
    ostringstream ostr;
    if (alpha() < 0xFF)
        ostr<< "#" << std::setw(2) << std::setfill('0') << std::uppercase << std::hex << red() 
...
void Color::setNamedColor(const std::string& name)
{
