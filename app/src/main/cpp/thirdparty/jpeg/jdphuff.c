        ...
        huffman_offset_data *offset)
{
  ...
  offset->insufficient_data = entropy->pub.insufficient_data ;
  offset->unread_marker = cinfo->unread_marker ;
}
        ...
        huffman_offset_data offset)
{
  ...
  cinfo->unread_marker = offset.unread_marker ;
  entropy->restarts_to_go = offset.restarts_to_go;
  entropy->pub.insufficient_data = offset.insufficient_data ;
...
jinit_phuff_decoder (j_decompress_ptr cinfo)
{
  ...
  #ifdef SKIA_MT_LIBJPEG_TILE_DECODE
    memset(cinfo->entropy , 0, SIZEOF(phuff_entropy_decoder) );
    entropy->pub.my_struct_size = SIZEOF(phuff_entropy_decoder) ;
  #endif  
